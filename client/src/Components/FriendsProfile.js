...
function FriendsProfile({ open, setOpen, profile }) {
  const classes = useStyles();
  const [imageFile, setImageFile] = useState({});

  const retrieveImage = useCallback(() => {
    axios
      .get(`${API}/viewprofile/${profile?.id}`)
      .then((resp) => {
        axios.get(`${API}/uploadfile/${resp.data.imageid}`).then((resp) => {
          setImageFile({
            file: resp.data?.file,
      ...
      >
        <DialogTitle id="customized-dialog-title" onClose={handleClose}>
          <List style={{ display: "flex", alignItems: "center" }}>
            ...
            <span style={{ marginLeft: "1rem" }}>{profile?.name}</span>
          </List>
        </DialogTitle>
        ...
        <Divider />
        <List>
          <ListItem button autoFocus onClick={handleClose} color="primary">
            Save changes
          </ListItem>
        </List>
      </Dialog>
